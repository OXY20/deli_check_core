...
- **单文件 / 目录扫描两种模式**：
  - 不传参数时，默认启动 Web 可视化界面。
  - 使用 `-f` 参数可指定单个 `.xls` 或 `.xlsx` 文件进行 CLI 处理。
- **空记录告警**：员工无任何打卡记录时会在日志中输出提示，但输出中仍保留空记录结构。
- **Web 可视化界面**：
  - 使用 `-port` 参数可以指定 Web 服务端口（默认为 8080）。
  - 浏览器上传考勤文件，在线查看打卡记录。
  - 支持多表信息合并模式：同时上传多个考勤表，自定义地点名称，保留全部原始记录并汇总统计。
  - 按人员显示打卡详情，支持用餐统计（早餐 / 午餐 / 晚餐）。
  - 一键导出用餐数据为 `.xlsx` 表格。
  - 支持导出人员明细数据（多表模式下含地点字段）。
  - **多表合并时员工编号冲突处理**：前端弹窗确认后的最终结果会与 `data/output/records.json` 保持同步，确保服务端落盘数据与展示/导出口径一致。
